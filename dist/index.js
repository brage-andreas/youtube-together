...
client.on("interactionCreate", async (intr) => {
    if (!intr.isChatInputCommand() || intr.commandName !== "youtube")
        return;
    if (!intr.guild)
        return intr.reply({ content: "This command can only be used inside a server.", ephemeral: true });
    ...
    if (!channelId)
        return intr.reply({ content: "You need to be in a voice channel.", ephemeral: true });
    ...
    if (invite.code === "50013")
        return void intr.editReply("I am missing permissions to launch activities or create invites.");
    intr.editReply(`Successfully made a Watch Together activity!\n[Click here to join.](https://discord.com/invite/${invite.code})`);
});
